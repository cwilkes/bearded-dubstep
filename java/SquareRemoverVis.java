...
public class SquareRemoverVis {
    ...
    private static final String SR_MOVES = System.getenv("sr_moves");
    public static final int ANSWER_STEPS = Integer.valueOf(SR_MOVES == null ? "10000" : SR_MOVES);
    ...
    public int[] runSolution(TestCase tc, String execCmd) throws Exception {
		...
		String val = "x";
			...
			for (int j=0 ; j < ans.length; j+=3) {
				System.out.println(String.format("Move %d : (%d,%d) %d", j/3, ans[j], ans[j+1], ans[j+2]));
}
                    throw new Exception(String.format("Unable to read element %d (0-based) of your solution's return value. '%s': %s: %s", i, val, e, java.util.Arrays.toString(ans)));
                }
            }
	...
	printBoard("solved", world.board);
        return world.score;
    }

   public static void printBoard(String label, String[] board) {
	// cjw
	String out = "";
	for (int r = 0; r < board.length; r++) {
		...
		for (int c = 0; c < board.length; c++) {
			out += String.valueOf(board[r].charAt(c));
		}
	}
	System.out.println("Board:(" + label + "): " + out);			
	}
